...
public class JSO {
    ...
    public JSO(InputStream json) throws malformedJSON{
        this("", TYPE_OBJECT, null, null, clean(json));
    }
    ...
    public JSO(String name, int TYPE, JSO root, JSO parent, String json) throws malformedJSON{
        ...
        switch(TYPE){
            ...
            case TYPE_STRING:
                ...
                break;
            case TYPE_NUMBER:
                this.data = Double.parseDouble(json);
                this.name = name;
                break;
            case TYPE_BOOLEAN:
...
//        json = clean(json);
    ...
    private static String clean(String json){
        ...
        String retjson = "";
        while(s.hasNext()){
            retjson += s.findInLine("[^\"\\s]+|\"(\\\\.|[^\\\\\"])*\"");
        }
        s.close();
        return retjson.replace("\t", "").replace("\n", "");
    }
    private static String clean(InputStream json){
        Scanner s = new Scanner(json);
        String retjson = "";
        while(s.hasNext()){
            retjson += s.findInLine("[^\"\\s]+|\"(\\\\.|[^\\\\\"])*\"");
    ...
    public JSO o(int node) throws NodeNotFound{return traverse(""+node);}
